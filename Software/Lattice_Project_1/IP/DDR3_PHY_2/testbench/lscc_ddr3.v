...
module lscc_ddr3 # (
    ...
    parameter DIFF                 = 0,
    parameter RANDOM_SEED          = 711689044,
    parameter RZQ                  = 240,
    ...
    parameter MAX_SIZE             = (1<<(BA_BITS+ROW_BITS+COL_BITS-BL_BITS)),
...
localparam RFF_BITS = DQ_BITS*BL_MAX;
 // %z format uses 8 bytes for every 32 bits or less.
localparam RFF_CHUNK = 8 * (RFF_BITS/32 + (RFF_BITS%32 ? 1 : 0));
...
begin : file_io_open
    if(MAX_MEM) begin
        ...
        for (bank_m = 0; bank_m < BANKS; bank_m = bank_m + 1)
            memfd[bank_m] = open_bank_file(bank_m);
    end
end
...
localparam SAME_BANK  = 2'd0; // same bank, same group
localparam DIFF_BANK  = 2'd1; // different bank, same group
localparam DIFF_GROUP = 2'd2; // different bank, different group
...
task data_task;
    ...
    begin
        ...
        if (rdq_cntr > 0) begin // read data
            if (mpr_en) begin
            if(MPR_DQ0) begin // DQ0 output MPR data, other DQ low				   
            ...
            else begin // all DQ output MPR data
                if (mpr_select == 2'b00) begin // Calibration Pattern
